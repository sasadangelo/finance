...
try:
    ...
    cur.executemany(
        "INSERT INTO etfs (Name, ISIN, Ticker, LaunchDate, Capital, Replication, Volatility, \
        Currency, Dividend, DividendFrequency, Yeld) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);",
        to_db,
    )
    ...
    for etf_quotes_csv in os.listdir("database/quotes"):
        ...
        cur.executemany(
            "INSERT INTO quotes (Ticker, Date, Open, High, Low, Close, Adj_Close, Volume) \
            VALUES (?, ?, ?, ?, ?, ?, ?, ?);",
            to_db,
        )
